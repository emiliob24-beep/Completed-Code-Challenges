...
func indexN(numbers: [Int], n: Int) -> Int? {
    guard n >= 0 && n < numbers.count else {
        return nil
    }
    let base = Double(numbers[n])
    let exponent = Double(n)
    
    return Int(pow(base, exponent))
}
print(indexN(numbers: [1, 2, 3, 4, 5, 6], n: 2)as Any)
