...
func testScores(myScore: Int, classScores:[Int]) -> Bool {
    let average = Double(classScores.reduce(0, +)) / Double(classScores.count)
    return Double(myScore) > average
}
testScores(myScore: 95, classScores: [90, 95, 100, 80])
testScores(myScore: 85, classScores: [90, 95, 100, 80])
