...
func phoneNumber(_ number: String) throws -> String {
    if number.isEmpty {
...
let phone = "123"
do {
    let result = try phoneNumber(phone)
    print(result)
} catch phoneError.empty {
    print("Phone Number is empty")
} catch phoneError.invalidNumber {
    print("Phone Number Invalid")
} catch {
    print("An unexpected error occurred: \(error)")
}
print(try phoneNumber("3853587170"))
