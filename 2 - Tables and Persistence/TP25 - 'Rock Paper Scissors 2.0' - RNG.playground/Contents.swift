...
func playRPS(input: String) -> String {
    let moves = ["rock", "paper", "scissors"]
    let userMove = input.lowercased()
    guard moves.contains(userMove) else {
        return "Invalid move! Use rock, paper, or scissors."
    }
    let computerMove = moves.randomElement()!
    if userMove == computerMove {
        return "I chose \(computerMove). Draw! Go again?"
    }
    switch (userMove, computerMove) {
    case ("rock", "scissors"), ("scissors", "paper"), ("paper", "rock"):
        return "I chose \(computerMove). I lose."
    default:
        return "I chose \(computerMove). You lose!"
    }
}
