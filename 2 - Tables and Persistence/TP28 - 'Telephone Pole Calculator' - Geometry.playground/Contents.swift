...
func distanceBetweenCenters(numberOfPillars: Int, distance: Int, diamter: Int) -> Double {
    let diameterInMeters = Double(diamter) / 100.0
    let gaps = numberOfPillars - 1
    let centerToCenter = Double(distance) + diameterInMeters
    
    return Double(gaps) * centerToCenter
}
let result = distanceBetweenCenters(numberOfPillars: 5, distance: 25, diamter: 10)
print(result)
