...
func sheepsAndWolves(sheep: [String]) -> String {
    guard let wolfIndex = sheep.firstIndex(of: "Wolf") else {
        return "No wolf found"
    }
    let positionFromFront = (sheep.count - 1) - wolfIndex
    
    if positionFromFront == 0 {
        return "Please go away and stop eating my sheep"
    } else {
        return "Oi, sheep number \(positionFromFront) you're about to be eaten!"
    }
}
sheepsAndWolves(sheep: ["Sheep", "Sheep", "Sheep", "Sheep", "Wolf", "Sheep"])
print(sheepsAndWolves(sheep: ["Sheep", "Sheep", "Sheep", "Sheep", "Wolf", "Sheep"]))
